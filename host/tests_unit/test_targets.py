...
class TestHeterogeneousPairAddressing:
    ...
    def test_the_compute_die_originates_receiver_keyed_cross_die_rules(self):
        ...
        # (0x41), the mailbox via aperture #2 (0x44 — not 0x42: 0x42/0x43 is the
        # M4 periph bit-band alias), so compute->eth SRAM is cam_rule(0x41, 0x2D)
        # and mailbox is cam_rule(0x44, 0x23).
        compute = get_target(COMPUTE)
        eth = get_target(ETH)
        assert compute.cam_rule_for("ipc_mailbox", far_target=eth) == 0x00234401
...
# shared_sram (0x41->0x2D) AND far-die ipc_mailbox (0x44->0x2A/0x23) at the same
# time. The mailbox byte is 0x44, not 0x42: 0x42/0x43 is the M4 peripheral
# bit-band alias, so the mailbox sits above it in the contiguous peer run.
# These tests pin that the registry now models per-region source bytes.
# =============================================================================
class TestSecondSourceAperture:
    def test_cam_rule_for_selects_the_mailbox_aperture_by_region(self):
        compute = get_target(COMPUTE)
        eth = get_target(ETH)
        # Mailbox leaves via aperture #2 (match 0x44 — not 0x42: 0x42/0x43 is the
        # M4 periph bit-band alias); replace is the RECEIVER's mailbox byte —
        # 0x2A on a compute receiver, 0x23 on an eth receiver.
        assert compute.cam_rule_for("ipc_mailbox", far_target=compute) == 0x002A4401
        assert compute.cam_rule_for("ipc_mailbox", far_target=eth) == 0x00234401
    ...
    def test_peer_selects_the_source_aperture_by_region(self):
        ...
        # Mailbox -> aperture #2, 0x44-based, so a doorbell rides its own window.
        assert compute.peer(0x0020, which="ipc_mailbox") == 0x44000020
        assert compute.peer(0, which="ipc_mailbox") == 0x44000000

    def test_is_peer_admits_the_whole_contiguous_run_on_both_links(self):
        # SAFETY CRITICAL: is_peer is the gate that forces a peer access onto the
        # FCSM==4 require_link_up check. The decoder runs NUM_PEER_BYTES=4, so the
        # WHOLE contiguous run 0x41..0x44 (0x61..0x64) decodes to hsel_peer ->
        # ahb_sub. If any byte in the run were not seen as peer, an access there
        # on a down link would escape the gate and hang the PS bus (H2). The dead
        # bytes 0x42/0x43 (0x62/0x63) — M4 periph bit-band, no CAM rule — must
        # still classify as peer for exactly that reason.
        compute = get_target(COMPUTE)
        for addr in (0x41000000, 0x42000000, 0x43000000, 0x44000000,  # link 0 run
                     0x61000000, 0x62000000, 0x63000000, 0x64000000):  # link 1 run
            assert compute.is_peer(addr) is True, (
                "is_peer(0x%08X) must be True — it is inside the peer run" % addr)
        # The run is BOUNDED: 0x45/0x65 are above it, so the decoder's anti-alias
        # boundary moved there and they are NOT peer.
        assert compute.is_peer(0x45000000) is False
        assert compute.is_peer(0x65000000) is False
    ...
    def test_the_sram_aperture_is_still_the_odd_anti_tx_half(self):
        ...
        # aperture that wedges. The mailbox aperture (0x44/0x64) sits higher in
        # the contiguous peer run, which is legal in the range-based decoder
        # (NUM_PEER_BYTES=4); its parity is not load-bearing.
        compute = get_target(COMPUTE)
...
class TestProvisionalComputeTarget:
    ...
    def test_it_declares_the_post_g4_link0_peer_aperture(self, target):
        ...
        # byte (0x44 link 0 / 0x64 link 1) so a doorbell reaches the far mailbox
        # CONCURRENTLY with SRAM on aperture #1 (they no longer contend for one
        # aperture). It is 0x44 not 0x42 because 0x42/0x43 is the M4 peripheral
        # bit-band alias; the mailbox sits above it, at the top of the contiguous
        # NUM_PEER_BYTES=4 run (0x42/0x43 and 0x62/0x63 are dead-but-decoded).
        assert target.peer_aperture_mbox == 0x44
